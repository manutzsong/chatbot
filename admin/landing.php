...
$sql_today ="SELECT SUM(`time`) AS 'time' FROM `today_intent` WHERE `intent_day` = DATE(NOW())";
$result_today = mysqli_query($con, $sql_today);

//top user
$sql_user = "SELECT * FROM `most` ORDER BY `time_usage` DESC LIMIT 1";
...
function stop_server() {
  ...
  <body style="height:100vh;" data-spy="scroll" data-target=".navbar" data-offset="50" onload="show_status();refresh();">
...
<div class="row">
	...
	<div class="col">
		...
		<div class="card text-center">
			<h5 class="card-header">Top User</h5>
			<div class="card-body">
			
			
			<p class="card-text"><?php while($row_user = mysqli_fetch_assoc($result_user)) { 
							$user_usage = $row_user['time_usage'];
							$user_who = $row_user['prof_name'];
							
				echo $user_who. '<br>' . $user_usage . ' time';
									}
							?></p>
...
\" data-toggle='popover' title='Intent Reply' data-content='Loading...' >$intent_name</a>
	...
	<footer class="footer head fixed-bottom">
      <div class="container">
        <span class="text-muted">MIS Chatbot</span>
      </div>
    </footer>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <button type="button" class="btn btn-primary" data-dismiss="modal" onClick="stop_server();">Stop Server</button>
      </div>
    </div>
  </div>
</div>
