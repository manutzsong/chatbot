...
$pre_query = "SELECT * FROM `answer_after`";
...
while($row = mysqli_fetch_assoc($query)) {
    ...
    $add_id = "INSERT INTO `word_count` (`id_primary_key`) VALUES ('$row_id');";
...
while($row_col = mysqli_fetch_assoc($query_colname)) {
  ...
  $query_sum_each = "select `$col_name` as each_count, `id_primary_key` from word_count";
  ...
  while($row_count_each = mysqli_fetch_assoc($query_wordname)) {
      ...
      if ($col_name != "id_primary_key" && $col_name != "its_cat") {
          ...
          $each_pri_id = $row_count_each['id_primary_key'];
          $run_wordtotal = "INSERT INTO `word_total` (`id_primary_key`, `word_name`, `word_cat`, `word_count`) VALUES (NULL, '$col_name','$each_pri_id', '$each_cat');";
